...
const user2  =  new User("Alex", "alex@example.com", Role.CUSTOMER);
...
const comment2 =  new Comment("Не коштуэ своїх грошей!!!", user2, 2);

product.addComment(comment1);
product.addComment(comment2);

console.log(product.getComments().map(c => `${c.getUser().getName()} сказал: "${c.getText()}", рейтинг: ${c.getRating()}`));
